...
Four weeks ago, I had an idea: build an AI advisory board where multiple AI experts could debate each other. Today, it's live at [boailoop.vercel.app](https://boailoop.vercel.app) with 695 automated tests, zero production incidents, and users getting simultaneous advice from 3-5 AI advisors.
...
## Sprint 1: Foundation (5 days) - Testing from Day One
...
## Sprint 2: Multi-Advisor Loops (7 days) - When Claude Code Caught What I Missed
...
## Sprint 3: Polish & Dashboard (7 days) - 200% Velocity Without Breaking Production
...
The standout achievement was E2E test refactoring. I had 14 failing E2E tests with brittle CSS selectors like `.button-primary:nth-child(2)`. Claude Code suggested a complete overhaul: replace CSS selectors with semantic `data-testid` attributes and create helper functions to reduce duplication.

The result? Code duplication reduced by 60%, test reliability improved to 100%, and the tests were re-enabled in CI. Claude Code generated the helper functions, I reviewed the patterns, and together we transformed a flaky test suite into a robust one.
...
## Sprint 4: MVP Feature Complete (8 days) - The Customer Pivot

Sprint 4 started with a plan: implement parallel LLM execution, add API versioning, and build advanced document management. Then the customer and I had a conversation that changed everything.

**We jointly decided to deprioritize three planned features:**

1. Advanced document management UI (simple upload was sufficient)
2. Parallel LLM execution (sequential was fast enough for 3-5 personas)
3. API versioning (premature for early-stage MVP)

**And prioritize three different features instead:**

1. Axiom logging (production monitoring and debugging)
2. Streaming UX improvements (eliminate jarring layout shifts)
3. 5-turn rate limiting (prevent API cost runaway)

This pivot delivered a better MVP. Production monitoring meant we could debug issues with real data. Streaming UX improvements made the product feel polished. Rate limiting gave us cost predictability for launch.
...
- Sprint 1: ~64 hours (13h/day - database setup and testing foundation)
- Sprint 2: ~78 hours (11h/day - multi-advisor orchestration)
- Sprint 3: ~72 hours (10h/day - dashboard and E2E refactoring)
- Sprint 4: ~92 hours (12h/day - production readiness and PDF processing)

**Late-night work:** Multiple debugging sessions until 2-3 AM. The PDF processing compatibility issues. Integration test stability. E2E test re-enablement. Rate limits research (10 hours of analysis to create a comprehensive strategy document).
...
**Worth noting:** The late-night work was effective but unsustainable. If you're building with AI assistance, pace yourself. The productivity gains are real, but they don't eliminate the need for deep focus and problem-solving.
...
3. **Integration test pattern** - Shared user pattern caused early flakiness. Test user pooling should have been the default.

4. **Late-night debugging** - Effective but unsustainable. Better time management would have prevented some of the crunch.
...
**Live at:** [boailoop.vercel.app](https://boailoop.vercel.app)
...
**Want to try Boailoop?** Check out [boailoop.vercel.app](https://boailoop.vercel.app)
