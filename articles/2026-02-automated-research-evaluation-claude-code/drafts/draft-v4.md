...
*How a bash loop, structured prompts, and Obsidian produced 371 auditable source citations across 10 platform evaluations*
...
Here's the part that makes people pause. The entire orchestration is eight lines:

```bash
while :; do
    output=$(cat evaluation_prompt.md | claude --dangerously-skip-permissions)
    echo "$output"
    if echo "$output" | grep -q "ALL SOLUTIONS EVALUATED"; then
      echo "Done — all solutions evaluated."
      break
    fi
  done
```
...
**Search quality varies.** Nextcloud produced 83 sources. LaSuite Docs produced 14. The framework can't create documentation that doesn't exist.
