...
*How a bash loop and structured prompts produced 371 auditable citations across 10 evaluations*
...
Doing this manually would mean a week or two of inconsistent research. I know myself: the first evaluation would be thorough. By the fifth, I'd be skimming product pages. By the tenth, I'd be scoring from memory and pretending I checked.
...
Those two define what to evaluate. The next three define how.

**3. Candidate List** — A markdown table that also serves as state management. The evaluation prompt reads the first row, removes it, evaluates that platform. When the table is empty, it stops. This makes the loop resumable — if something crashes at platform 7, platforms 1-6 are already gone from the list.
...
Here's the part that makes people pause. The entire orchestration is six lines:
...
The `--dangerously-skip-permissions` flag lets Claude Code write files and run web searches without approval prompts — required when running without supervision.
...
**Why separate contexts.** Context overflow kills quality in long AI tasks. One session evaluating all 10 platforms would degrade — later evaluations get less attention, information bleeds between platforms, scores drift. The loop gives each evaluation a fresh context with full attention.

**Why it's resumable.** If evaluation #7 fails, run the loop again. Platforms #1-7 are already removed. Claude picks up at #8. No checkpoint database, no orchestration framework — just a file that gets shorter.

**What it produced.** Ten evaluation files, 650-980 lines each. Each file includes YAML frontmatter with all 18 scores, detailed criterion-by-criterion justifications with numbered source citations, 11 scenario assessments, and 6 hypothesis validations. Total across all evaluations: approximately 73,000 words and 371 source citations.

Scores ranged from 130 to 221 out of 280 possible points — average 174. The framework isn't flattering anyone.
...
**Automated evaluation: ~15 minutes per platform.** Each loop iteration — web searches, page fetches, scoring, writing the evaluation file — took about 15 minutes. Ten platforms: roughly 2.5 hours of automated runtime, about $2 per evaluation in API costs.

**The manual alternative.** Thoroughly evaluating one platform against 18 weighted criteria with source citations — reading official docs, checking third-party reviews, writing up scored justifications — realistically takes a full day. Ten platforms at that pace means two weeks. But the real cost of manual evaluation isn't the time — it's the corners you cut. I'd have evaluated fewer platforms, checked fewer sources, and made do with less evidence. And I'd still have the consistency problem.

**Total: 8 hours of human effort plus 2.5 hours of machine time for 10 complete evaluations with 371 source citations.**

---

## Challenges

That said, the framework has clear boundaries.
